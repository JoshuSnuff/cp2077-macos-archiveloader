...
restore_pristine() {
    ...
    # Patcher backups are deliberately left alone here: this runs mid-session,
    # and the current run's backups are the only way back from a failed patch.
    # restore_archives.sh clears them once the session is over.
